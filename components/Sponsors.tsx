...
const sponsors: Sponsor[] = [
  ...
  { name: "Non-Trivial", src: "/sponsors/nontrivial.png", url: "https://apply.non-trivial.org/referral-network", tier: "Gold" },
...
const TierSection = ({ tier, color, data }: { tier: SponsorTier; color: string; data: Sponsor[] }) => {
  ...
  return (
    <div className="flex flex-wrap justify-center gap-4 sm:gap-6 md:gap-12 w-full max-w-6xl mb-12 md:mb-16">
      {data.map((s, idx) => (
        <a
          key={idx}
          href={s.url}
          ...
          className={`group relative flex items-center justify-center bg-white p-3 md:p-6 rounded-xl transition-all duration-300 border border-gray-100 shadow-sm hover:-translate-y-2
            ${tier === 'Gold' ? 'w-[140px] h-[90px] sm:w-[200px] sm:h-[120px] md:w-[300px] md:h-[180px]' : ''}
            ${tier === 'Silver' ? 'w-[120px] h-[80px] sm:w-[180px] sm:h-[110px] md:w-[260px] md:h-[140px]' : ''}
            ${tier === 'Bronze' ? 'w-[100px] h-[70px] sm:w-[150px] sm:h-[90px] md:w-[220px] md:h-[120px]' : ''}
          `}
          onMouseEnter={(e) => {
...
export default function Sponsors() {
  ...
  return (
    <div className="flex flex-col items-center py-10 px-5 w-full">
      <h2 className="text-3xl md:text-4xl font-black text-[#002E67] mb-12 md:mb-16 tracking-tight text-center">
      ...
      <div className="mt-4 text-[#002E67]/60 text-sm font-bold text-center">
        Interested in sponsoring? Contact us at <a href="mailto:ghsmathcircle@gmail.com" className="underline hover:text-[#002E67] transition-colors break-all">ghsmathcircle@gmail.com</a>
      </div>
    </div>
  );
}